...
test("summarizeBashCommand avoids semantic labels for pipelines", () => {
	assert.deepEqual(summarizeBashCommand("find . -name '*.ts' | sort"), {
		text: "find . -name '*.ts' | sort",
		summarized: false,
	});
	assert.deepEqual(summarizeBashCommand("rg -n needle src | wc -l"), {
		text: "rg -n needle src | wc -l",
		summarized: false,
	});
	assert.deepEqual(summarizeBashCommand("rg -n 'needle|other' src"), {
		text: "rg /needle|other/ in src",
		summarized: true,
	});
