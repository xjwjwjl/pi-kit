...
test("uses actual quota durations and never reports an unavailable quota as available", () => {
	...
	const report = formatReport({
		...
		shortRange: null,
		longRange: null,
		shortStats: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, cost: 0, sessions: 0, models: {} },
		longStats: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, cost: 0, sessions: 0, models: {} },
	...
	assert.match(report, /Codex Usage · unknown · unavailable/);
	assert.doesNotMatch(report, /scan /);
	assert.doesNotMatch(report, /· available/);
});
