...
if (!agentDir || !donePath || !["session-start", "credential-changed", "retry"].includes(scenario ?? "")) {
	...
	ctx = {
		...
		signal: undefined,
		ui: {
			setStatus(_key: string, status: string | undefined) { if (status?.includes("%")) finish(status); },
	...
	codexUsage({
		...
		events: { on(name: string, handler: () => void) { eventHandlers.set(name, handler); } },
		registerCommand() {},
	} as never);
	...
	if (!sessionStart) process.exitCode = 3;
		...
		try {
			...
			if (scenario === "credential-changed") {
				...
				if (!credentialChanged) process.exitCode = 4;
				else {
					await new Promise((resolve) => setTimeout(resolve, 400));
					writeFileSync(join(agentDir, "auth.json"), JSON.stringify({
						"openai-codex": { type: "oauth", access: "test-access-token", expires: originalDateNow() + 60 * 60 * 1000 },
					}), "utf-8");
					credentialChanged();
				}
			}
