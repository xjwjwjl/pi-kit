...
import { hasUsagePercent, selectStatusWindow, windowBoundary } from "./quota.ts";
import type { CacheEntry, CodexCredential, PersistentCache, UsageResult } from "./types.ts";
...
export const CACHE_FILE_NAME = "codex-usage-cache.json";
...
export function credentialCacheKey(credential: CodexCredential | string): string {
...
function normalizeResult(value: unknown): UsageResult | undefined {
	if (!value || typeof value !== "object") return undefined;
	const raw = value as { status?: unknown; usage?: unknown; error?: unknown };
	if (raw.status !== "success" && raw.status !== "expired"
		&& raw.status !== "error" && raw.status !== "cancelled") return undefined;
	if (raw.usage !== null && (typeof raw.usage !== "object" || Array.isArray(raw.usage))) return undefined;
	return {
		status: raw.status,
		usage: raw.usage as UsageResult["usage"],
...
export function savePersistentCache(
	credential: CodexCredential | string,
	entry: CacheEntry,
	paths = getQuotaCachePaths(),
): PersistentCache {
...
export function isSuccessfulUsage(result: UsageResult): boolean {
	return result.status === "success"
		&& Boolean(result.usage && hasUsagePercent(selectStatusWindow(result.usage)));
}
...
export function createCacheEntry(
	...
	const successful = isSuccessfulUsage(result);
	const failed = result.status === "error" || (result.status === "success" && !successful);
	const storedResult: UsageResult = failed
		? { status: "error", usage: null, error: result.error || "quota unavailable" }
		: result;
