...
function usageCost(usage: any): number {
	...
	if (cost && typeof cost === "object" && !Array.isArray(cost)) {
		return firstNumber(cost.total) ?? 0;
	}
...
function findSessionFiles(root: string, earliestStartMs: number): SessionFileCandidates {
	...
	while (directories.length > 0) {
		...
		for (const entry of entries) {
			...
			totalFiles += 1;
			try {
				// Session files are append-only in normal Pi operation. A file whose
				// mtime predates the oldest window cannot contain recent messages.
...
export async function scanSessionsInRanges(
	...
	if (!ranges.some((range) => Boolean(range)) || !fs.existsSync(sessionsDir)) {
	...
	for (const filePath of candidates.files) {
		...
		try {
			...
			for await (const line of rl) {
				...
				if (obj.type !== "message" || stringField(obj, "role") !== "assistant") continue;
				if (stringField(obj, "provider") !== "openai-codex") continue;
				...
				entries.push({
					entryId: sessionEntryId(obj),
					model: sessionModel(obj),
					usage,
					indexes,
				});
			}
